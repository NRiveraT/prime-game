...
func _ensure_ao_mask(size: Vector2i) -> void:
	if _ao_mask_tex.is_valid() and _last_size == size:
		return
	if _ao_mask_tex.is_valid():
		_rd.free_rid(_ao_mask_tex)
...
func _render_callback(p_callback_type: int, p_render_data: RenderData) -> void:
	...
	# RTShadowEffect also calls sm.tick() — if both effects run in the same frame,
	# only the first call advances the state machine. That is intentional and safe.
	sm.tick()
	...
	u_mask_read.add_id(_ao_mask_tex)

	var comp_set := _rd.uniform_set_create(
		[u_color, u_mask_read], _comp_shader, 0
	)

	# apply_rtao push constant: single float ao_strength
	var comp_push := PackedFloat32Array([ao_strength])
